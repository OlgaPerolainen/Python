...
def change_config():
    ...
    """
    config_file = r'source/config.py'
    with open(config_file, "r") as f:        
        data = f.read()
        data = data.replace(f"username = '{username}'", "username = 'OlgaP'") \
            .replace(f"password = '{password}'", "password = 'rsb4243'") \
            .replace(f"server = '{server}'",
                     "server = 'LAPTOP-9NI9ES32\SQLEXPRESS'") \
            .replace(f"database = '{database}'", "database = 'Farmers'")
