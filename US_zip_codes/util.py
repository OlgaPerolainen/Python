...
def read_zip_all():
    ...
    for line in open('zip_codes_states.csv').read().split("\n"): #file name can be given to a variable in the beginning of the function
        skip_line = False
        m = line.strip().replace('"', '').split(",")     #what is m? entry?         
        i += 1
        if i == 1:
            for val in m:
                header.append(val)                                      
            ...
            for idx in range(0, len(m)):
                if m[idx] == '':
                    skip_line = True
                    break
                if header[idx] == "latitude" or header[idx] == "longitude": #names can be given to variables in the beginning of the function
