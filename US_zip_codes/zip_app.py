...
as well as to calculate the distance between to ZIP codes.          #two
...
def calculate_distance(location1, location2):
    ...
    del_lat = (lat1 - lat2) / 2                              #add separate function
    del_long = (long1 - long2) / 2                           #for a repeated code
    angle = math.sin(del_lat)**2 + math.cos(lat1) * math.cos(lat2) * \
        math.sin(del_long)**2
    distance = 2 * 3959.191 * math.asin(math.sqrt(angle))    #magic number. earth_radius_miles = 3959.191
    return distance

                                                             #add comments to other 7 functions

def degree_minutes_seconds(location):                        #can be local function for def format_location(location)
    minutes, degrees = math.modf(location)
    degrees = int(degrees)
    minutes *= 60                                            #magic number MINUTES_IN_DEGREE = 60
    seconds, minutes = math.modf(minutes)
    minutes = int(minutes)
    seconds = 60 * seconds                                   #seconds *= 60; magic number: seconds_in_minute = 60
    return degrees, minutes, seconds


def format_location(location):
    ns = ""                                          #write in a separate function def cardinal_directions(location) 
    if location[0] < 0:
        ns = 'S'
    elif location[0] > 0:                            #what if would be = 0?
        ns = 'N'

    ew = ""
    if location[1] < 0:                              #what if would be = 0?
        ew = 'W'
    elif location[0] > 0:                            #location[0] or location[1]?     
        ew = 'E'                                     
                                                                             #US ia always N and W
    format_string = '{:03d}\xb0{:0d}\'{:.2f}"'
    latdegree, latmin, latsecs = degree_minutes_seconds(abs(location[0]))    #lat_degree, lat_min, lat_secs
    latitude = format_string.format(latdegree, latmin, latsecs)              #lat_degree, lat_min, lat_secs
    longdegree, longmin, longsecs = degree_minutes_seconds(abs(location[1])) #long_degree, long_min, long_secs
    longitude = format_string.format(longdegree, longmin, longsecs)          #long_degree, long_min, long_secs
...
def zip_by_location(codes, location):
    zips = []
    for code in codes:
        if location[0].lower() == code[3].lower() and \  #magic numbers
           location[1].lower() == code[4].lower():       #maybe won't work with another DB
            zips.append(code[0])
    return zips


def location_by_zip(codes, zipcode):                     #add function to validate zip code entered by user
    for code in codes:
        if code[0] == zipcode:                           #magic numbers, maybe won't work with another DB
            return tuple(code[1:])                       #better to return tuple with all data found by the code
    return ()


def process_loc(codes):
    zipcode = input('Enter a ZIP Code to lookup => ')    #add .strip() or replace(" ", "")
    print(zipcode)
    location = location_by_zip(codes, zipcode)
    if len(location) > 0:
        print('ZIP Code {} is in {}, {}, {} county,\ncoordinates: {}'.
              format(zipcode, location[2], location[3], location[4], #magic numbers
                     format_location((location[0], location[1]))))   #maybe won't work with another DB
    else:
...
def process_zip(codes):
    ...
    print(city)
    city = city.strip().title()                                #one string with input() city = input('Enter a city name to lookup => ').strip().title() 
    state = input('Enter the state name to lookup => ')
    print(state)
    state = state.strip().upper()                              #one string with input() state = input('Enter the state name to lookup => ').strip().upper()  
...
def process_dist(codes):
    zip1 = input('Enter the first ZIP Code => ')         #add .strip() or replace(" ", "")                
    print(zip1)
    # logging.info(f'Received the first ZIP {zip1}')
    logger.info(f'Received the first ZIP {zip1}')
    zip2 = input('Enter the second ZIP Code => ')        #add .strip() or replace(" ", "")                  
    print(zip2)
    # logging.info(f'Received the second ZIP {zip2}')
    logger.info(f'Received the second ZIP {zip2}')

    location1 = location_by_zip(codes, zip1)              #location1 gets the tuple with all the data found by zip code, not only latitude and longitude
    location2 = location_by_zip(codes, zip2)              #location2 gets the tuple with all the data found by zip code, not only latitude and longitude
    if len(location1) == 0 or len(location2) == 0:
        print('The distance between {} and {} cannot be determined'.
              format(zip1, zip2))
    else:
        dist = calculate_distance(location1, location2)         #add indexes to location1, location2 not to transmit to the function a tuple with all the data found by zip code and make sure that indexes of the function match indexes of latitude and longitude
...
if __name__ == "__main__":
    ...
    command = ""
    while command != 'end':
        command = input("Command ('loc', 'zip', 'dist', 'end') => ") #ambiguous commands "loc" and "zip"
        # logging.info(f'Received command {command}')
        logger.info(f'Received command {command}')
        print(command)
        command = command.strip().lower()
        if command == 'loc':                             #better use numbers to avoid misprints
            ...
            print("Invalid command, ignoring")           #all instructions could be added from another module that contains them in different languages
        print()
    print("Done")
    logging.shutdown()
